...
export function estimateCostPerHour(
	...
	return (
		workers * (cpu * FARGATE_VCPU_PER_HOUR + memoryGb * FARGATE_GB_PER_HOUR)
	);
}
...
export function printProcessing(total: number, workers: number): void {
	stderr(
		`📊 Processing ${total} item${total === 1 ? "" : "s"} across ${workers} worker${workers === 1 ? "" : "s"}`,
	);
}

export function printChunks(chunks: number, avg: number): void {
	stderr(
		`📦 ${chunks} chunk${chunks === 1 ? "" : "s"} (avg ${avg.toFixed(1)} items each)`,
	);
}
