...
import { estimateCost, estimateCostPerHour } from "./cost.js";
...
export function parseMemoryGb(memory: string): number {
	...
	if (upper.endsWith("GB")) {
		return Number.parseFloat(upper.slice(0, -2));
	}
	if (upper.endsWith("MB")) {
		const mb = Number.parseFloat(upper.slice(0, -2));
		return Math.max(1, Math.ceil(mb / 1024));
	}
	// Plain number assumed to be GB
	return Number.parseFloat(memory);
}
