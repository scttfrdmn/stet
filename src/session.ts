...
import { decodeResult, encodeTaskFile } from "./serialize.js";
...
export class Session {
	...
	async run(
		...
		const ratePerHour = estimateCostPerHour(
			this._cpu,
			this._memoryGb,
			this._workers,
		);
		...
		if (totalVcpu > quotaVcpu) {
			...
			printQuotaWarning(
				...
				totalVcpu,
				usedWorkers,
				usedWorkers * this._cpu,
			);
			throw new BurstQuotaError(
				actualWorkers,
				usedWorkers,
				"fargate_vcpu",
				quotaVcpu,
	...
	async _pollUntilDone(
		...
		return new Promise((resolve, reject) => {
			const interval = setInterval(async () => {
				...
				try {
					const { done, failed } = await this._countStatuses(
						s3,
						sessionId,
						chunkCount,
					);
...
export class DetachedSession {
	...
	async status(): Promise<SessionStatus> {
		...
		const manifest = JSON.parse(buf.toString("utf-8")) as Record<
			string,
			unknown
		>;
		return {
	...
	async collect(timeout?: number, signal?: AbortSignal): Promise<unknown[]> {
		...
		const chunkCount = (manifest as unknown as Record<string, unknown>)
			.tasksTotal as number;
		...
		const rawManifest = JSON.parse(buf.toString("utf-8")) as Record<
			string,
			unknown
		>;
		const totalChunks =
			(rawManifest.chunk_count as number | undefined) ?? manifest.tasksTotal;

		const ratePerHour = manifest.costEstimatePerHour;
		const startTime = Date.now();
		const deadline =
			timeout !== undefined ? startTime + timeout * 1000 : undefined;
...
export async function attach(
	...
	const resolvedCfg = cfg ?? (await (await import("./config.js")).loadConfig());
	return new DetachedSession({ sessionId, cfg: resolvedCfg });
}
