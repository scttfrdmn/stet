...
const SESSION_ID = requireEnv("BURST_SESSION_ID");
const TASK_ID = requireEnv("BURST_TASK_ID");
const S3_BUCKET = requireEnv("BURST_S3_BUCKET");
...
async function putS3(key: string, body: Buffer | string): Promise<void> {
	...
	await s3.send(
		new PutObjectCommand({ Bucket: S3_BUCKET, Key: key, Body: buf }),
	);
}
