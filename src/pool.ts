...
export class Pool {
	...
	private _lastFn: ((...args: unknown[]) => unknown) | null = null;

	constructor(options?: BurstOptions) {
		this._options = options ?? {};
	}

	async map<T, U>(items: T[], fn: (item: T) => Promise<U> | U): Promise<U[]> {
		if (this._shutdown) {
		...
		const fnCast = fn as (...args: unknown[]) => unknown;
		if (!this._bundleResult || this._lastFn !== fnCast) {
			const { bundleFunction } = await import("./bundle.js");
			this._bundleResult = await bundleFunction(fnCast);
			this._lastFn = fnCast;
